...
def assp_branch(in_channels, out_channles, kernel_size, dilation):
    ...
    return nn.Sequential(
            ...
            # nn.ReLU(inplace=True) # original, commented by @dv
            nn.ReLU(inplace=False) # @dv
            )
...
class ASSP(nn.Module):
    def __init__(self, in_channels, output_stride):
        ...
        self.scales = [0.75, 1.0, 1.25, 1.5, 1.75, 2.0, 2.25] # added by @dv
        initialize_weights(self)
    ...
    def forward(self, x):
        ...
        for scale in self.scales:
            m = nn.Upsample(scale_factor=scale)
            if scale == self.scales[0]:
                ...
                x1 += F.interpolate(self.aspp1(m(x)), size=(x1.size(2), x1.size(3)), mode='bilinear', align_corners=True)
                x2 += F.interpolate(self.aspp2(m(x)), size=(x2.size(2), x2.size(3)), mode='bilinear', align_corners=True)
                x3 += F.interpolate(self.aspp3(m(x)), size=(x3.size(2), x3.size(3)), mode='bilinear', align_corners=True)
                x4 += F.interpolate(self.aspp4(m(x)), size=(x4.size(2), x4.size(3)), mode='bilinear', align_corners=True)
...
class DeepLab(BaseModel):
    def __init__(self, num_classes, in_channels=3, backbone='xception', pretrained=True, 
        ...
        # super(DeepLab, self).__init__() # original, commented @dv
        super().__init__() # @dv: error fiz
