...
class CReSISDataset(BaseDataSet):
    def __init__(self, **kwargs):
        ...
        super(CReSISDataset, self).__init__(**kwargs)

    def _set_files(self):
        # if self.split in ['train_split', 'val', 'test', 'train']:
        file_list = os.path.join(self.root, self.split + '_pair_semantic.txt')
        self.files = [name.rstrip() for name in tuple(open(file_list, "r"))]
        # else: raise ValueError(f"Invalid split name {self.split} choose one of [trainSplit, val, test]")
    ...
    def _load_data(self, index):
        ...
        # image = np.float32(cv2.fastNlMeansDenoisingColored(cv2.imread(image_path),None,5,5,7,7)) # @dv: added for denoinsing
        label = np.asarray(Image.open(label_path), dtype=np.int32) # @dv: had been using this till now
        # image = cv2.imread(image_path) # @dv: copied from dr. yari's code on 06/25/20
        # label = cv2.imread(label_path, cv2.IMREAD_GRAYSCALE) # @dv: copied from dr. yari's code on 06/25/20
        return image, label, image_id



class CReSIS(BaseDataLoader):
    def __init__(self, data_dir, batch_size, split, crop_size=None, base_size=None, scale=True, num_workers=1, val=False,
        ...
        self.dataset = CReSISDataset(**kwargs)
