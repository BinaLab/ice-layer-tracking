...
def main():
    ...
    assert dataset_type in ['VOC', 'COCO', 'CityScapes', 'ADE20K', 'CReSIS']
    ...
    # list_dir for loop created by @dv
    # root = 'G:/My Drive/Debvrat_Research/Dataset'
    root = 'G:/My Drive/Debvrat_Research/Dataset/Snow Radar'
    root_path = os.path.join(root,args.dataset)
    list_dir = os.listdir(root_path)
    out_main = os.path.join(args.output, network_timestamp.group(1), args.dataset)
    for folder in list_dir:
        # if folder!= 'image':
        #     continue
        if os.path.isdir(os.path.join(root_path,folder)):
            args.output = os.path.join(out_main, folder) # @dv
            if not os.path.exists(args.output):
                os.makedirs(args.output)
            image_files = sorted(glob(os.path.join(root_path,folder,'image', f'*.png')))
            # image_files = sorted(glob(os.path.join(root_path,folder, f'*.png')))
    # image_files = sorted(glob(os.path.join(args.images, f'*.{args.extension}'))) # original @dv
    # image_files = sorted(glob(os.path.join('2012_cropped',args.data_type,'image', f'*.png'))) #@dv: was using this earlier
    
            print('....Predicting '+folder+' set....')        
            with torch.no_grad():
                ...
                for img_file in tbar:
                    ...
                    save_images(image, prediction, args.output, img_file, palette) # color palette - commented by @dv
                    # plt.imsave(os.path.join(args.output,os.path.basename(img_file)),prediction.astype(np.uint8)) # to generate viridis palette @dv

def parse_arguments():
    parser = argparse.ArgumentParser(description='Inference')
    parser.add_argument('-c', '--config', default='config_cresis.json',type=str,
                        help='The config used to train the model')
    parser.add_argument('-mo', '--mode', default='multiscale', type=str,
                        help='Mode used for prediction: either [multiscale, sliding]')
    parser.add_argument('-m', '--model', default='saved/DeepLab/01-08_13-08/best_model.pth', type=str, 
    ...
    parser.add_argument('-o', '--output', default='G:/My Drive/Debvrat_Research/Outputs/Snow Radar/Semantic Segmentation', type=str,  
                        help='Output Path')
