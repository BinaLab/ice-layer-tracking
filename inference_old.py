...
def main():
    ...
    args.output = os.path.join(args.output, network_timestamp.group(1), args.data_type) # @dv
    if not os.path.exists(args.output): # @dv
        os.makedirs(args.output)
    # image_files = sorted(glob(os.path.join(args.images, f'*.{args.extension}'))) # original @dv
    image_files = sorted(glob(os.path.join('2012_cropped',args.data_type,'image', f'*.png')))
...
def parse_arguments():
    ...
    parser.add_argument('-m', '--model', default='saved/PSPNet/07-17_03-37/best_model.pth', type=str,
