...
def eval_metrics(output, target, num_classes):
    correct, labeled = batch_pix_accuracy(output.data, target) # original, commented by @dv for dsn
    inter, union = batch_intersection_union(output.data, target, num_classes) # original, commented by @dv for dsn
    # correct, labeled = batch_pix_accuracy(output[0].data, target) # @dv: to carry out dsn
    # inter, union = batch_intersection_union(output[0].data, target, num_classes) # @dv: to carry out dsn
