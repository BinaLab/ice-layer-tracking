...
class Log(_LRScheduler):
    def __init__(self, optimizer, num_epochs, iters_per_epoch=0, warmup_epochs=0, last_epoch=-1):
        ...
        self.warmup_iters = warmup_epochs * iters_per_epoch
        self.factor = 1 #@dv
        super(Log, self).__init__(optimizer, last_epoch)

    def get_lr(self):
        T = self.last_epoch * self.iters_per_epoch + self.cur_iter
        factor = self.factor

        if T!=0 and (T%(10*self.iters_per_epoch) == 0): # divide factor by 10 after every 10th epoch
            factor = factor/10
            self.factor = factor
            
        # factor = factor*math.exp(-self.last_epoch/20)
        # self.factor = factor
