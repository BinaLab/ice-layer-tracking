...
def save_images(image, mask, output_path, image_file, palette):
    ...
    w, h, _ = image.shape # @dv: for opencv
...
def main():
    ...
    network_timestamp = re.search('/(.+?)/checkpoint-epoch100', args.model) # @dv
    ...
    root = 'G:/My Drive/Debvrat_Research/Dataset/RDS/' # @dv: windows
    # root = '/Volumes/GoogleDrive/My Drive/Debvrat_Research/Dataset/RDS/' # @dv: mac
    
    # rds_types = ['MacGregor_200721', 'MacGregor_200824/frames_001_016_20120507_05', 
    #              'MacGregor_200824/frames_001_016_20120508_06', 
    #              'MacGregor_200824/frames_001_018_20120508_04', 
    #              'MacGregor_200824/frames_001_028_20120507_07',
    #              'MacGregor_200824/frames_001_030_20120330_03']
    rds_types = ['MacGregor_200824/frames_001_016_20120507_05', 
             'MacGregor_200824/frames_001_016_20120508_06', 
             'MacGregor_200824/frames_001_018_20120508_04', 
             'MacGregor_200824/frames_001_028_20120507_07',
             'MacGregor_200824/frames_001_030_20120330_03']
    for rds in rds_types:
        ...
        image_files = sorted(glob(os.path.join(root_path,'image_cropped', f'*.tiff')))
        ...
        with torch.no_grad():
            ...
            for img_file in tbar:
                ...
                image = cv2.imread(img_file, -1) # @dv: for reading tiff files
                if image is None:
                    print(img_file+' gives None')
                    continue
                ### previous exp @ dv ###
                # image = (image + 2.335165500640869) / (1.0+ 2.335165500640869) # @dv
                ...
                ### exp1-10/4/2020 ^ ###
                image = cv2.cvtColor(image,cv2.COLOR_GRAY2RGB) # @dv: to convert to 3 channels, as required by the trained model
...
def parse_arguments():
    ...
    parser.add_argument('-c', '--config', default='config_rds.json',type=str,
                        help='The config used to train the model')
    parser.add_argument('-mo', '--mode', default='multiscale', type=str,
                        help='Mode used for prediction: either [multiscale, sliding]')
    # parser.add_argument('-m', '--model', default='saved/DeepLab/10-02_00-59/checkpoint-epoch50.pth', type=str, # @dv: rds model path
                        # help='Path to the .pth model checkpoint to be used in the prediction')
    # parser.add_argument('-m', '--model', default='saved/DeepLab/08-14_11-02/best_model.pth', type=str, # @dv: snow radar model path
    #                     help='Path to the .pth model checkpoint to be used in the prediction')
    parser.add_argument('-m', '--model', default='saved/DeepLab/10-04_15-40/checkpoint-epoch100.pth', type=str, # @dv: snow radar model path
                    help='Path to the .pth model checkpoint to be used in the prediction')
    ...
    parser.add_argument('-o', '--output', default='G:/My Drive/Debvrat_Research/Outputs4/RDS/SemanticSegmentation', type=str,  
                        help='Output Path')
    parser.add_argument('-e', '--extension', default='tiff', type=str, # @dv: author's default = 'jpg'
